...
- `AI_IMPLEMENT_FORWARD_SECRETS` — repo/org **Actions variable** (not a secret). Comma-separated list of repository secret names to inject into the runner environment. Forwarded secrets reach setup and verify hooks only — the runner strips them from Claude's subprocess environment before the agent spawns, so they are never visible to the Claude agent. Names must match `^[A-Z][A-Z0-9_]*$`, must not start with `RUN_` or `AI_IMPLEMENT_`, and must not collide with the runner's own credential names (`GITHUB_TOKEN`, `GH_TOKEN`, `GITHUB_ENTERPRISE_TOKEN`, `GH_ENTERPRISE_TOKEN`, `GIT_PASSWORD`, `AI_IMPLEMENT_APP_ID`, etc.).
