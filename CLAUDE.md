...
`workflows/claude-plan.yml` is the planning workflow synced to target repos. It is a thin container shim that boots the shared runner image (`entrypoint.sh` with `RUNNER_PHASE=planning`), so planning runs identically on GitHub Actions, Fly Machines, and local Docker — there is no separate `anthropics/claude-code-action` invocation for planning. Bedrock planning is GHA-only (Fly Machines and local Docker lack the OIDC role-assumption mechanism required for `sts:AssumeRoleWithWebIdentity`). The runner performs read-only codebase analysis, writes structured planning output to `ai-output/comments/`, and the orchestrator posts those files as comments to Linear. It supports:
