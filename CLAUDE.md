...
**Jira repo matching derives from the mapping's `owner`/`repo`.** A Jira mapping's `repoFieldValue` is optional (`null` by default) and resolved at read time through `resolveRepoFieldValue` (`src/providers/ticketing-config.ts`): blank means "match issues whose repo field equals `owner/repo`", so the GitHub repository is entered once in the admin UI and never duplicated into the JQL or a separate repo-value field. Set `repoFieldValue` explicitly only when the repo field's options are labelled with something other than `owner/repo`. **Do not add a repo clause to the JQL** - the orchestrator filters candidates by the repo field client-side (it tolerates both option-object and bare-string serializations, which a JQL `=` does not), so a JQL repo clause is redundant and can silently drop issues; the admin JQL validator flags one as an advisory. The capacity query (Planning/Implementing) filters by the same resolved value, so a per-mapping cap counts only that mapping's in-flight issues even when several mappings share one project-scoped JQL.
