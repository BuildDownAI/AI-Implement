...
| `reviewers` | Advisory prompt-only reviewer definitions read from the checked-out workspace. Each entry needs `id` and `prompt`; `model` is optional. Definitions carry no `gates` field, so selecting/gating reviewers stays in project settings and the run envelope. Built-in and image-baked reviewer definitions take precedence over matching config ids |
