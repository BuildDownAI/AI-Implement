...
After the refresh PR is open, `kg-snapshot-push` classifies the run and, for exactly one matching class, posts a second comment on that PR whose first line is the exact marker `# ai-implement-kg-refresh-learnings` — the marker `kg_ingest/cards.py` (in the KG base repo) matches to file a comment as a Learning card, so a stamped refresh becomes searchable as a lesson, not just a diff. The four classes, checked in this order, are: `guard-refused` (the push guard reported a non-clean verdict — structurally unreachable today, since both content-regression guards throw before any push or PR exists; kept for a future guard that reports a verdict instead of throwing), `source-missing` (a secondary repo configured in `sources.yml` has no cloned checkout under `repos/`, or the configured `code_repo` was cloned but its checkout under `code-repo/` has no content — a code-repo clone *failure* aborts the pipeline before this step runs, since `clone-code-repo` throws rather than soft-failing, so only the "came back empty" half of the code-repo case is reachable here), `count-step` (a part's line count moved more than 20% against the served snapshot in either direction — looser than the push guard's 50%-shrink refusal, so it also fires on runs that already cleared that guard), and `ingest-warnings` (`ai-output/kg-ingest.log` has a `WARN` or `ERROR` line). A clean run matches none of these and posts nothing. Reading these comments to decide whether a sanitized note goes to `bd-knowledge-graph-base` is skills-side work (`bd-kg-refresh` Step 8), tracked separately.
...
**The harness also fetches the `kg-scope-reconcile` diff input itself, independently of `--tracker-data` (AII-604 gap-fill).** In a real dispatch, `kg-scope-reconcile` (the step right after `clone`, § 5) fetches the orchestrator's mapping list over the runner callback (`POST /api/runner/kg-scope`, progress-token authenticated) — but the harness's kg-refresh container has no live orchestrator to call back to, and never wires `RUNNER_CALLBACK_URL`/`RUN_PROGRESS_TOKEN`, so without this the step's dry-run diff was unreachable through the documented local workflow. Whenever `ORCHESTRATOR_URL` plus an admin credential are set — regardless of whether `--tracker-data` was also given — `startDevRun` additionally calls `GET /api/kg/scope` (the admin-authenticated mirror of the runner route, gated by an admin session rather than a progress token, same `{teamKey, repo, defaultBranch, ticketingProvider}` shape) and writes the result to `.dev-runs/<timestamp>/kg-scope.json` before bind-mounting it at `/dev-kg-scope.json`. Unlike tracker data, this is never required: with no orchestrator env, or a failed fetch, the harness logs it and moves on — `kg-scope-reconcile` then hits its ordinary "no callback URL" skip inside the container. `AI_IMPLEMENT_KG_DRY_RUN=true` (set for every local kg-refresh run) makes the step print the would-be diff and write nothing to `sources.yml`.
...
- When fetched (see above), the KG-scope file is bind-mounted at `/dev-kg-scope.json`; `kg-scope-reconcile` detects `KG_SCOPE_FILE` and diffs against it rather than calling `POST /api/runner/kg-scope`. Absent the orchestrator env, no mount is made and the step skips exactly as it would on a real dispatch with no callback configured.
