...
Skills find the server at session start with a ToolSearch for `get_project_binding` (no `CLAUDE.md` key names it; `kg.mcp_server` is retired), then discover tools by description, so a new read tool is usable the day it ships. A skill that performs a declared write is an **admin-only skill**: it calls `get_session_identity` first and stops with a clear message when the role is not `admin`. Read-only skills never call a write tool, whatever the role.
