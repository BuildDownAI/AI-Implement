...
- **AII-843: restate-server 1.7.10 never reports the conflict `register()` was written to expect.** CI first showed the pair above failing (`registered-no-force` where `declined-conflict` was expected) because the original version of `endpoint.restate.test.ts` dropped the unrelated `orchestratorTools` service while the in-flight invocation was pinned to `Operator`. Fixing which service gets dropped wasn't enough: confirmed directly against the pinned `@restatedev/restate-server` 1.7.10 platform binary this repo already depends on (same version string and git commit the pinned Docker image reports), a no-force `POST /deployments` to an already-registered URI is accepted unconditionally — HTTP 200, no live discovery performed — no matter how far the live endpoint has drifted from the stored manifest, including the actively-invoked-service-dropped case itself, a fully unreachable endpoint, and an incompatible handler-type swap. `force: true` is the only path that performs live discovery and applies a changed manifest. `register()` (`src/restate/endpoint.ts`) no longer waits for the admin API's HTTP status to signal drift: on every successful no-force call it now compares the admin API's confirmed manifest against `RegisterDeps.services` (defaulting to `RESTATE_SERVICES`) and treats a mismatch the same as a real META0004 conflict, before running the same drain-check-then-force logic the function already had. `endpoint.restate.test.ts` passes the post-swap service set explicitly for this reason — the module-level `RESTATE_SERVICES` no longer reflects what the test's swapped endpoint actually serves. `restate-endpoint.test.ts` (unit tier) covers the comparison itself: a matching manifest stays a single no-force call, a mismatched one escalates through the same drain/force path a real META0004 does.
