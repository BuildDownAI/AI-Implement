...
`register()` (`src/restate/endpoint.ts`) posts the deployment without `force` at boot: an unchanged endpoint answers 200/201. On restate-server 1.7.10, a *changed* service set at an already-registered URI also answers 200/201 without `force` — confirmed directly against the pinned `@restatedev/restate-server` platform binary this repo depends on (AII-843): a no-force `POST /deployments` to a known URI is accepted unconditionally, without live discovery, regardless of how far the live endpoint has drifted, so the admin API never itself reports the `META0004` conflict this mechanism was originally written expecting. `register()` therefore detects the drift client-side — comparing the manifest the admin API just confirmed against `RegisterDeps.services` (defaulting to `RESTATE_SERVICES`) — and treats a mismatch the same as a real conflict (the 409/`META0004` branch is still handled, in case a different server version or registration path does surface one). Either way, `force: true` overrides the deployment but "can lead inflight invocations to an unrecoverable error state" (Restate's own guidance), so it is used only after confirming zero non-completed invocations on the deployment it would replace. Never force a registration to get past a conflict.
