...
`post-push-review.ts` names the configured cap in its own PR comment; `run-autonomous.ts` names it again in the ticket-facing `failureReason` it builds and reports as `failureCode: "REVIEWER_TURNS_EXHAUSTED"` with the record attached, and `formatRunAutopsy` (`src/run-autopsy.ts`) renders the same sentence with an open-PR (not draft-PR) line. `formatFailureComment` (`src/runner-callback.ts`) currently renders that `failureReason` as-is through its generic branch — the reviewer ran out of turns, it did not reject the code, so it must never fall into `REVIEW_UNAPPROVED`'s "did not approve" wording.
