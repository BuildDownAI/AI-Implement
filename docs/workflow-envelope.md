...
**Legacy dispatches keep using `findWorkflowRunId`; pilot callers must not.** The existing post-dispatch heuristic (`src/github.ts`) scans the workflow's recent-runs list for a run created at or after the dispatch time, on the expected branch, optionally disambiguated by `display_title` against `issueIdentifier` — because `workflow_dispatch` alone never returns a run id, this scan is how every current caller (`src/index.ts`'s implementation/gap-analysis dispatch and monitor path, `monitor-gha.ts`, the kg-refresh dispatch) resolves one today. None of these callers pass `returnRunDetails`; a `200` with a validated body would narrow *when* the scan is needed (a caller already holding `runId`/`runUrl` could skip it), but a `204` — the outcome a plain dispatch actually gets — still carries no identity, so for this population the heuristic remains the only mechanism.

That is a statement about today's legacy callers, not a license for the Restate review-fix pilot to reuse the same heuristic once it starts dispatching. AII-770's worker port draws the line explicitly: `ReviewFixWorkerPort.reconcile` (`src/review-fix-ports.ts`) must look up an attempt's execution "by exact identity (`attemptId` scoped to `scope`), never by a fuzzy or time-windowed match," and must report `"unknown"` — not `"not_found"` — whenever the lookup is inconclusive, because zero or multiple candidates does not prove no worker exists. `findWorkflowRunId`'s title/time-window match is precisely the kind of fuzzy match that contract forbids as proof of identity. A pilot dispatch that gets back `accepted` (the 204 case, or an unvalidated 200) has *not* thereby resolved which run it started; `outcome: "accepted"` tells the caller GitHub queued something, not which run id owns it. A pilot caller in that state has unresolved identity and must reconcile through the worker port, not fall back to the legacy scan. This reconciliation path has no live binding yet — the worker port's GitHub Actions adapter is AII-793 — so today no pilot caller consumes `returnRunDetails` at all; the outcome contract above is wire-format groundwork for that adapter, not something exercised by a shipped caller yet.
...
`WorkflowCapabilities` carries a `supportsAttemptCorrelation` bit, detected the same way as `supportsRunPublicationToken`: `RUN_ATTEMPT_TOKEN_RE` (`/^\s{2,}run_attempt_token:\s*$/m`) line-matches the fetched YAML *text* for an indented `run_attempt_token:` key. This is a regex marker over raw text, not a YAML parse that structurally verifies the key sits under `on.workflow_dispatch.inputs` — the probe never loads a YAML library or walks a parsed document, here or for `run_config`/`run_publication_token`. It is `true` only when the contract itself resolved to `"envelope"` and the regex matched; a `"legacy"` result (or any probe failure) forces it `false` regardless of what the YAML contains.

Publishing that input in `workflows/claude-implement.yml` is AII-782's job, not this repo's current state: as of this branch, no synced template declares `run_attempt_token`, so `supportsAttemptCorrelation` probes `false` for every repo until AII-782 lands and repos re-sync the template. The detection mechanism itself is not speculative — `resolveWorkflowCapabilities` already computes and caches the bit correctly today against whatever text it's given — but the *name* it matches is provisional too: `RUN_ATTEMPT_TOKEN_RE`'s own comment (`src/workflow-probe.ts`) notes no concrete workflow input name existed at authoring time, so it mirrors `run_publication_token`'s naming as a placeholder, and AII-782/AII-776 may publish a different name — in which case this regex, not just the template, needs updating to match. Treat `run_attempt_token` here as "the name this repo currently probes for," not as an input AII-782 has committed to.
