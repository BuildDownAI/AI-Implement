...
These are written down and, more importantly, enforced in code rather than left as tribal knowledge. The ten rules are collected in `docs/restate.md` § "Working with Restate: patterns and pitfalls". Three of them are guardrails a future migration inherits for free: the `tool()` wrapper handles the empty body and the retried-error/suspension distinction (`src/restate/tools.ts`), `POST /api/tools/<name>`'s name regex plus `encodeURIComponent` closes the path-traversal class (`src/admin.ts`, `src/restate/tools-client.ts`), and `writeIdempotencyKey` names one request so a retry deduplicates but a new session runs (`src/mcp.ts`). **Net: a positive** — a rule enforced by a wrapper is a rule the next author cannot forget.
