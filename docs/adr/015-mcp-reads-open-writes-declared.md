...
The "one declared write list in `src/mcp.ts`" mechanism described above ended with [AII-713](https://linear.app/eudoxus/issue/AII-713/migrate-the-declared-mcp-writes-into-the-tools-service-as-durable): the six writes are now handlers on the `orchestratorTools` Restate service (`src/restate/tools.ts`), each declaring `role: "admin"` in the same `mcp.type`/`mcp.role` metadata a read tool already carries (AII-710/AII-711), and the role check runs inside the shared `tool()` wrapper rather than a `/mcp`-only lookup. `WRITE_TOOLS`, `WriteTool`, and the adapter-local `run()` bodies are gone; `src/mcp.ts` keeps only a courtesy `tools/list` filter and a routing set (`RESTATE_TOOL_NAMES`) that no longer distinguishes reads from writes for dispatch purposes. The audit line moved with the check, into the wrapper, so a write reached through `POST /api/tools/<name>` or `callToolAsSystem` (AII-712) is logged too — before, only a call through `/mcp` was. Each write is additionally invoked with a Restate idempotency key derived from the caller and the MCP request id, so a client retry of the same call attaches to the first run instead of re-executing it.

The decision's rule is unchanged: a tool is a read unless it declares an admin role; a write's declaration is the single source of truth for whether it exists and who may call it; refusal is a tool result, never a JSON-RPC error. Only *where* the declaration lives moved — from a hand-maintained array to handler metadata discoverable the same way a read tool's shape already was.
