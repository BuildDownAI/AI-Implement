...
**`review_fix_queue`'s pre-existing internal producers — `guardOpenPrBeforeImplementationDispatch` (`src/index.ts`, reason `open_pr`) and `handleLeaseRejectedFailure` (`src/runner-callback.ts`, reason `lease_rejected`) — call `enqueueReviewFix` directly and are unaffected.** They carry no `sourceEventId` (`source_event_id` stays `NULL` on their audit rows) and keep relying on the queue's own `(repo, pr_number)` coalescing described above, not this per-event identity. `/ai-implement` (`enqueueCommentGapfill`, `src/comment-gapfill-queue.ts`) is a separate queue entirely and is untouched by any of this.
