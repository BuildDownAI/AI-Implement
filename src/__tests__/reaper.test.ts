...
describe("sweepOrphanedKgRefreshJobs — GHA rows skipped by reaper", () => {
  it("GHA kg-refresh row is not acted upon regardless of age or runId", async () => {
    const ghaJob = {
      ...kgRefreshJob,
      id: 30,
      ...
      machineId: null,
      runId: null,
      dispatchedAt: Date.now() - 10 * 60_000, // 10 min old — past the old grace window
    };
    vi.mocked(listMachines).mockResolvedValueOnce([] as never);
    vi.mocked(getJobByMachineId).mockReturnValue(undefined);
  ...
  it("GHA kg-refresh row with runId is still skipped (monitor owns its lifecycle)", async () => {
    const ghaJob = {
      ...kgRefreshJob,
      id: 31,
      executionMode: "github-actions",
